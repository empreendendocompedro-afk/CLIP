...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    raw_segs    = []
    seg_start   = 0.0
    cur_anchor  = snap(smoothed[0][1]) if has_real_face_near(smoothed[0][0]) else mid

    for i in range(1, len(smoothed)):
        t, cx      = smoothed[i]
        # Se não há rosto real próximo, usa centro
        if not has_real_face_near(t):
            new_anchor = mid
        else:
            new_anchor = snap(cx)
    ...
    # Mesclar segmentos curtos com o vizinho (sem fazer média — mantém anchor)
    segments = []
    for seg in raw_segs:
        dur = seg["end"] - seg["start"]
        if dur < MIN_DUR and segments:
            segments[-1]["end"] = seg["end"]   # absorve, mantém cx anterior
        else:
