...
def download_video(url, job_id, settings):
    ...
    # X-Forwarded-For com IP brasileiro — bypass geobloqueio regional
    base = ["yt-dlp", "--no-playlist", "--no-check-certificates",
            "--merge-output-format", "mp4", "-o", tmpl,
            "--add-header", "X-Forwarded-For:177.75.0.1",
            ]
...
def detect_faces_timeline(video_path, clip_start, clip_end, sample_interval=0.5):
    """
    Detecta rostos em frames amostrados do clipe.
    Retorna lista de {time, faces: [{cx, cy, w, h}]} por frame amostrado.
    Usa OpenCV Haar Cascade — leve, sem GPU, funciona no Railway free tier.
    """
    try:
        import cv2
    except ImportError:
    ...
    while t <= clip_end:
        ...
        # Equalizar histograma melhora detecção em vídeos escuros/baixa qualidade
        gray = cv2.equalizeHist(gray)

        faces = face_cascade.detectMultiScale(
            gray,
            scaleFactor=1.05,   # mais preciso (era 1.1)
            minNeighbors=3,     # mais sensível (era 5)
            minSize=(20, 20),   # detecta rostos menores (era 30x30)
            flags=cv2.CASCADE_SCALE_IMAGE
        )

        # Se não achou com frontal, tenta perfil
        if len(faces) == 0:
        ...
        timeline.append({"time": round(t - clip_start, 2), "faces": face_list})
        t += sample_interval
...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    """
    Face tracking simplificado e robusto:
    1. Amostra frames a cada 0.5s
    2. Detecta maior rosto em cada frame
    3. Suaviza posições com mediana (resistente a outliers)
    4. Corta o vídeo em segmentos por posição de rosto
    5. Concatena com ffmpeg concat
    """
    try:
        import cv2
        import numpy as np
    ...
    # Scale de saída (máximo 2x o crop original)
    if aspect == "9:16":
        out_w = min(720,  crop_w * 2); out_w -= out_w % 2
        out_h = min(1280, crop_h * 2); out_h -= out_h % 2
    elif aspect == "1:1":
        out_w = out_h = min(720, min(crop_w, crop_h) * 2)
        out_w -= out_w % 2; out_h -= out_h % 2
    else:
        out_w = min(1280, crop_w * 2); out_w -= out_w % 2
        out_h = min(720,  crop_h * 2); out_h -= out_h % 2

    # ── Amostrar frames com detecção de movimento labial ────────
    cap      = cv2.VideoCapture(str(video_path))
    duration = clip_end - clip_start
    interval = 0.5
    samples  = []  # (time_offset, cx) — cx do rosto com mais movimento labial

    # Cascades para boca (detecta quem está falando)
    mouth_cascade_path = cv2.data.haarcascades + "haarcascade_smile.xml"
    mouth_cascade = cv2.CascadeClassifier(mouth_cascade_path)

    prev_mouth_regions = {}  # face_cx → região da boca no frame anterior
    LIP_MOVE_THRESH = 15     # diferença mínima de pixels para considerar movimento
    ...
    while t <= clip_end:
        ...
        # Margem de 5% nas bordas — exclui equipamentos colados nas bordas
        edge_margin = int(orig_w * 0.05)
        valid_faces = [(fx, fy, fw, fh) for (fx, fy, fw, fh) in faces
                       if fw >= 35 and fh >= 35
                       and fx > edge_margin
                       and (fx + fw) < (orig_w - edge_margin)
                       and fy < orig_h * 0.80
                       and (fy + fh * 0.5) < orig_h * 0.88]

        if not valid_faces:
            samples.append((round(t - clip_start, 2), None))
            t += interval
            continue

        if len(valid_faces) == 1:
            # Só um rosto — usa ele diretamente
            fx, fy, fw, fh = valid_faces[0]
            cx = int(fx + fw // 2)
            samples.append((round(t - clip_start, 2), cx))
        else:
            # Múltiplos rostos — detecta quem está com boca se movendo
            best_cx    = None
            best_score = -1
            ...
            for (fx, fy, fw, fh) in valid_faces:
                ...
                # Região da boca: terço inferior do rosto
                ...
                # Calcular movimento em relação ao frame anterior
                lip_score = 0
                key = (fx // 20)  # chave por posição aproximada
                if key in prev_mouth_regions:
                    prev = prev_mouth_regions[key]
                    if prev.shape == mouth_region.shape:
                        diff = cv2.absdiff(mouth_region, prev)
                        lip_score = int(np.mean(diff))

                prev_mouth_regions[key] = mouth_region.copy()

                # Pontuação = movimento labial + tamanho do rosto (proximidade)
                size_score = fw * fh
                total_score = lip_score * 3 + size_score // 100
            ...
            if best_cx is not None:
                samples.append((round(t - clip_start, 2), best_cx))
            else:
                # Fallback: maior rosto
                fx, fy, fw, fh = max(valid_faces, key=lambda f: f[2]*f[3])
                samples.append((round(t - clip_start, 2), int(fx + fw // 2)))
    ...
    if pct < 0.08:
        job_update(job_id, **{f"face_{clip_idx}_status": f"poucos rostos ({int(pct*100)}%) — crop central"})
        return False

    # ── Preencher lacunas (interpolação) ────────────────────────
    # Usa o cx anterior quando frame sem rosto
    filled = []
    last_cx = face_samples[0][1] if face_samples else orig_w // 2
    for t, cx in samples:
        if cx is not None:
            last_cx = cx
        filled.append((t, last_cx))

    # ── Suavizar com mediana de janela 5 ────────────────────────
    cx_values = [cx for _, cx in filled]
    smoothed  = []
    W = 5
    for i, (t, _) in enumerate(filled):
        window = cx_values[max(0, i-W):i+W+1]
        smooth_cx = int(sorted(window)[len(window)//2])  # mediana
        smoothed.append((t, smooth_cx))

    # ── Calcular âncoras por cluster (esquerda/direita) ──────────
    mid = orig_w // 2

    real_faces      = [(t, cx) for t, cx in face_samples]
    left_positions  = [cx for _, cx in real_faces if cx < mid]
    right_positions = [cx for _, cx in real_faces if cx >= mid]

    anchor_left  = int(sum(left_positions)  / len(left_positions))  if left_positions  else None
    anchor_right = int(sum(right_positions) / len(right_positions)) if right_positions else None

    if anchor_left  is None: anchor_left  = anchor_right
    if anchor_right is None: anchor_right = anchor_left

    # Se os âncoras estão muito perto das bordas (< edge_margin), deslocar para dentro
    # Garantir que âncoras não ficam na borda absoluta (min 10% de margem)
    edge_min = int(orig_w * 0.10)
    if anchor_left  is not None: anchor_left  = max(edge_min, anchor_left)
    if anchor_right is not None: anchor_right = min(orig_w - edge_min, anchor_right)

    if anchor_left  is None: anchor_left  = orig_w // 4
    if anchor_right is None: anchor_right = orig_w * 3 // 4

    # ── Construir segmentos usando só os anchors ──────────────
    # Cada ponto suavizado é "snapped" para o anchor mais próximo
    # Isso elimina posições de centro falsas
    MIN_DUR = 5.0   # duração mínima por segmento

    # Criar índice rápido de quais timestamps têm rosto real
    real_face_times = set(round(t, 1) for t, _ in face_samples)

    def snap(cx):
        dl = abs(cx - anchor_left)
        dr = abs(cx - anchor_right)
        return anchor_left if dl <= dr else anchor_right

    def has_real_face_near(t_check, window=2.0):
        """Verifica se há rosto real detectado perto deste timestamp."""
        return any(abs(rt - t_check) <= window for rt in real_face_times)

    # Construir segmentos: quando sem rosto real, mantém último anchor conhecido
    raw_segs  = []
    seg_start = 0.0

    # Usar primeira detecção real como anchor inicial (não a posição padrão)
    first_real_cx = next((cx for _, cx in face_samples), None)
    if first_real_cx is not None:
        last_anchor = snap(first_real_cx)
    else:
        last_anchor = anchor_left or anchor_right or mid
    cur_anchor = last_anchor

    for i in range(1, len(smoothed)):
        t, cx  = smoothed[i]
        if has_real_face_near(t):
            new_anchor  = snap(cx)
            last_anchor = new_anchor   # atualiza último anchor real
        else:
            new_anchor = last_anchor   # mantém último anchor conhecido (não centro)

        if new_anchor != cur_anchor:
            raw_segs.append({"start": seg_start, "end": t, "cx": cur_anchor})
            seg_start  = t
            cur_anchor = new_anchor

    raw_segs.append({"start": seg_start, "end": duration, "cx": cur_anchor})

    # Mesclar: segmentos curtos + segmentos consecutivos com mesmo cx
    segments = []
    for seg in raw_segs:
        dur = seg["end"] - seg["start"]
        if segments and segments[-1]["cx"] == seg["cx"]:
            # Mesmo anchor — estender em vez de criar novo segmento
            segments[-1]["end"] = seg["end"]
        elif dur < MIN_DUR and segments:
            # Segmento curto — absorve no anterior
            segments[-1]["end"] = seg["end"]
        else:
    ...
    for si, seg in enumerate(segments):
        ...
        # X do crop: centraliza no rosto, clampado nas bordas
        cx  = seg["cx"]
        x   = cx - half_w
        x   = max(0, min(orig_w - crop_w, x))
        x   = x - (x % 2)

        # Y do crop: sempre 0 para vídeos onde crop_h == orig_h
        # Para vídeos com margem vertical, sobe 15% para dar testa
        y = 0
        if orig_h > crop_h:
            cy_center = orig_h // 2
            y = max(0, cy_center - half_w - int(crop_h * 0.15))
            y = min(orig_h - crop_h, y)
            y = y - (y % 2)

        vf = f"crop={crop_w}:{crop_h}:{x}:{y},scale={out_w}:{out_h}:flags=lanczos,setsar=1"
        abs_start = clip_start + seg["start"]

        seg_path = tmp_dir / f"s{si:03d}.mp4"
        r = subprocess.run([
            "ffmpeg", "-y",
            "-ss", str(abs_start), "-i", str(video_path),
            "-t", str(seg_dur),
            "-vf", vf,
        ...
        if seg_path.exists() and seg_path.stat().st_size > 500:
            ...
            job_update(job_id, **{f"face_{clip_idx}_seg{si}":
                f"cx={cx} x={x} y={y} dur={round(seg_dur,1)}s"})
    ...
    if len(seg_files) == 1:
        import shutil
        shutil.copy(seg_files[0], str(out_path))
    else:
