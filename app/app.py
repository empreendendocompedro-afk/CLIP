...
def whisper_status():
    """Diagnóstico do Whisper."""
    try:
        import whisper
        whisper_ok = True
        whisper_version = getattr(whisper, "__version__", "?")
    except Exception as e:
        return jsonify({"whisper_installed": False, "error": str(e)})
    ...
    try:
        ...
        return jsonify({"whisper_installed": True, "torch_error": str(e)})
...
def transcribe_whisper(audio_path, job_id):
    """
    Transcrição local com Whisper (open-source, gratuita).
    Sem custo de API, sem limite de uso, sem depender de terceiros.
    """
    try:
        import whisper
        import torch
    except ImportError as ie:
        return None, [], [], f"Whisper não instalado: {ie}"

    try:
        job_update(job_id, message="🎙️ Carregando modelo Whisper...", 
                   whisper_model=WHISPER_MODEL)
        # Log disponibilidade de memória
        import os
        mem_info = ""
        try:
            with open("/proc/meminfo") as f:
            ...
            mem_avail = lines.get("MemAvailable", "?")
            mem_info = f" (RAM disponível: {mem_avail})"
        except: pass
        job_update(job_id, whisper_mem=mem_info)

        model = get_whisper_model()
        if not model:
            return None, [], [], f"Falha ao carregar modelo Whisper{mem_info}"

        job_update(job_id, message=f"🎙️ Transcrevendo com Whisper ({WHISPER_MODEL})...")

        # Transcrever com detecção automática de idioma e timestamps por palavra
        result = model.transcribe(
            str(audio_path),
            task="transcribe",
            word_timestamps=True,       # timestamps por palavra para legendas
            verbose=False,
            fp16=False,                 # CPU mode (Railway não tem GPU)
        ...
        job_update(job_id, whisper_lang=detected_lang)

        # Converter segmentos Whisper → formato interno
        raw_segs = []
        words_data = []
        ...
        for seg in result.get("segments", []):
            ...
            # Extrair palavras com timestamps para legendas
            for w in seg.get("words", []):
                words_data.append({
                    "text":  w.get("word", "").strip(),
                    "start": int(w.get("start", 0) * 1000),  # ms
        ...
        job_update(job_id,
            ...
            message=f"🎙️ Transcrição concluída — {len(words_data)} palavras detectadas"
        )
        return full_text, raw_segs, words_data, None

    except Exception as e:
        return None, [], [], f"Whisper erro: {e}"
