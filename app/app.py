...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    face_model = mp_face.FaceDetection(
        model_selection=0,       # 0=curta distância (até 2m), 1=longa
        min_detection_confidence=0.6  # só rostos com 60%+ de confiança
    )
    ...
    while t <= clip_end:
        ...
        # Converter detecções para coordenadas pixel
        faces_px = []
        for det in results.detections:
            bb   = det.location_data.relative_bounding_box
            fx   = int(bb.xmin * orig_w)
            fy   = int(bb.ymin * orig_h)
            fw   = int(bb.width  * orig_w)
            fh   = int(bb.height * orig_h)
            conf = det.score[0]
            # Filtrar rostos muito pequenos ou fora do frame útil
            if fw < 20 or fh < 20:
                continue
            if fy + fh * 0.5 > orig_h * 0.90:  # rosto no rodapé = provável artefato
                continue
        ...
        if len(faces_px) == 1:
            ...
            for face in faces_px:
                ...
                score = lip_score * 5 + int(face["conf"] * 100) + fw * fh // 300
                if score > best_score:
                    best_score = score
                    best_cx    = cx_face
    ...
    for si, seg in enumerate(segments):
        ...
        subprocess.run([
            ...
            "-threads", "2", "-c:a", "aac", "-b:a", "128k", "-ar", "44100",
