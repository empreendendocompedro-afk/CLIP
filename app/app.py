...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    while t <= clip_end:
        ...
        # Margem mínima de 8% das bordas — rostos reais não ficam colados nas bordas
        edge_margin = int(orig_w * 0.08)
        valid_faces = [(fx, fy, fw, fh) for (fx, fy, fw, fh) in faces
                       if fw >= 40 and fh >= 40
                       and fx > edge_margin                        # não na borda esquerda
                       and (fx + fw) < (orig_w - edge_margin)     # não na borda direita
                       and fy < orig_h * 0.75
                       and (fy + fh * 0.5) < orig_h * 0.85]
    ...
    edge_margin = int(orig_w * 0.12)
    if anchor_left  is not None: anchor_left  = max(edge_margin * 2, anchor_left)
    if anchor_right is not None: anchor_right = min(orig_w - edge_margin * 2, anchor_right)

    # Fallback: se ainda None, usar posições padrão de locutor típico (1/4 e 3/4 do frame)
    ...
    # (não usa centro como fallback — isso causava "puxão" para o meio)
    raw_segs      = []
    seg_start     = 0.0
    last_anchor   = anchor_left or anchor_right or mid  # anchor inicial
