...
def download_video(url, job_id, settings):
    ...
    # Flags para resolver o n-challenge do YouTube via Deno + script remoto
    ejs_flags = ["--remote-components", "ejs:github", "--js-runtimes", "deno"]

    strategies = [
        # 1) Web + Deno + script remoto (melhor compatibilidade com cookies)
        base + ejs_flags + ["--extractor-args", "youtube:player_client=web", "-f", fmt, url],
        # 2) TV embedded + Deno (funciona sem cookies em muitos casos)
        base + ejs_flags + ["--extractor-args", "youtube:player_client=tv_embedded", "-f", "best[height<=1080]/best", url],
        # 3) Fallback sem Deno — tenta baixar como stream direto
        base + ["-f", "best", "--no-check-formats", url],
    ]
...
def cut_clips(video_path, clips, job_id, settings, words_data):
    ...
    for i, clip in enumerate(clips):
        ...
        # Filtro de vídeo
        vf = []
        # Limitar resolução ao dobro do source para economizar RAM
        # (escalar 640x360 → 1080p não agrega qualidade e usa muita memória)
        max_h = min(orig_h * 2, 1080)  # nunca mais que 2x o original
        if aspect == "9:16":
            target_h = min(1280, max_h * 2)  # 9:16 vertical
            target_w = int(target_h * 9 / 16)
            cw = min(orig_w, int(orig_h * 9 / 16))
            vf += [f"crop={cw}:{orig_h}:(iw-{cw})/2:0",
                   f"scale={target_w}:{target_h}:flags=bilinear"]
        elif aspect == "1:1":
            sq = min(orig_w, orig_h)
            target_sq = min(720, sq * 2)
            vf += [f"crop={sq}:{sq}:(iw-{sq})/2:(ih-{sq})/2",
                   f"scale={target_sq}:{target_sq}:flags=bilinear"]
        else:
            target_w = min(1280, orig_w * 2)
            target_h = min(720,  orig_h * 2)
            vf += [f"scale={target_w}:{target_h}:flags=bilinear:force_original_aspect_ratio=decrease",
        ...
        cmd1 = (["ffmpeg","-y",
                 ...
                 "-c:v","libx264",
                 "-preset","ultrafast",   # mínimo de RAM — Railway free tier
                 "-crf",crf,
                 "-profile:v","baseline", # baseline usa menos memória que high
                 "-level","3.1",
                 "-pix_fmt","yuv420p",
                 "-threads","1",          # 1 thread = muito menos RAM
                 "-c:a","aac","-b:a","128k","-ar","44100",
                 "-bufsize","512k",       # buffer menor
                 ]
