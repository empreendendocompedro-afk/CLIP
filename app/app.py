...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    # ── Amostrar frames e detectar rostos ───────────────────────
    cap      = cv2.VideoCapture(str(video_path))
    duration = clip_end - clip_start
    interval = 0.5
    samples  = []  # lista de (time_offset, cx) — cx=None se sem rosto
    ...
    while t <= clip_end:
        ...
        valid_faces = [(fx, fy, fw, fh) for (fx, fy, fw, fh) in faces
                       if fw >= 40 and fh >= 40          # tamanho mínimo real
                       and fy < orig_h * 0.75            # rosto não pode estar no rodapé
                       and (fy + fh * 0.5) < orig_h * 0.85]  # centro do rosto acima de 85%
