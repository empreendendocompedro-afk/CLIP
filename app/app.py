...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    # ── Detectar quebras de posição (novo locutor) ──────────────
    # Limiar maior = menos trocas falsas; duração mínima = sem micro-cortes
    JUMP    = orig_w * 0.30   # 30% da largura (era 18%) — só trocas reais de locutor
    MIN_DUR = 4.0              # segmento mínimo de 4s para evitar micro-cortes

    raw_segs = []
    seg_start   = 0.0
    seg_cx_list = [smoothed[0][1]]

    for i in range(1, len(smoothed)):
        t, cx = smoothed[i]
        prev_cx = smoothed[i-1][1]
        if abs(cx - prev_cx) > JUMP:
            avg_cx = int(sum(seg_cx_list) / len(seg_cx_list))
            raw_segs.append({"start": seg_start, "end": t, "cx": avg_cx})
            seg_start   = t
            seg_cx_list = [cx]
        else:
            seg_cx_list.append(cx)

    if seg_cx_list:
        avg_cx = int(sum(seg_cx_list) / len(seg_cx_list))
        raw_segs.append({"start": seg_start, "end": duration, "cx": avg_cx})

    # Mesclar segmentos curtos com o vizinho de cx mais próximo
    segments = []
    for seg in raw_segs:
        dur = seg["end"] - seg["start"]
        if dur < MIN_DUR and segments:
            # Absorver no segmento anterior
            prev = segments[-1]
            # Recalcular cx como média ponderada pela duração
            total = (prev["end"] - prev["start"]) + dur
            prev["cx"] = int((prev["cx"] * (prev["end"] - prev["start"])
                               + seg["cx"] * dur) / total)
            prev["end"] = seg["end"]
        else:
            segments.append(dict(seg))

    # Garantir que o último segmento vai até o fim do clipe
    if segments:
