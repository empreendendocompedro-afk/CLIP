...
def smart_silence_removal(input_path, output_path, job_id, clip_index):
    try:
        ...
        # Calcular duração acumulada de cada segmento para o offset do xfade
        seg_durs = []
        for sf in seg_files:
            pr = subprocess.run(["ffprobe","-v","quiet","-print_format","json",
                                 "-show_format", sf], capture_output=True, text=True)
            try:
                d = float(json.loads(pr.stdout).get("format",{}).get("duration", 1))
            except:
                d = 1.0
            seg_durs.append(d)

        fc, vchain, achain = [], "[0:v]", "[0:a]"
        acc = 0.0  # tempo acumulado para o offset do xfade
        for i in range(1, n):
            acc += seg_durs[i-1] - crossfade  # offset = fim do segmento anterior
            vout = "vout" if i == n-1 else f"xv{i}"
            aout = "aout" if i == n-1 else f"xa{i}"
            fc.append(f"{vchain}[{i}:v]xfade=transition=fade:duration={crossfade}:offset={round(acc,3)}[{vout}]")
...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    """
    Face tracking em 3 fases:
    1. Scan inicial: encontra TODOS os rostos sem filtros de borda
    2. Tracking: segue o rosto com mais movimento labial (quem fala)
    3. Render: corta sub-clipes por posição e concatena
    """
    try:
        import cv2
        import numpy as np
    ...
    if aspect == "9:16":
        ...
        sq    = min(720, min(crop_w, crop_h) * 2)
        out_w = out_h = sq - (sq % 2)
    else:
        out_w = min(1280, crop_w * 2); out_w -= out_w % 2
        out_h = min(720,  crop_h * 2); out_h -= out_h % 2

    # ── FASE 1: Scan sem filtros — encontra todos os rostos ─────
    cap      = cv2.VideoCapture(str(video_path))
    duration = clip_end - clip_start
    samples  = []   # (t_offset, cx_do_locutor)
    prev_mouths = {}  # key → region do frame anterior para calcular movimento
    interval = 0.5
    t = clip_start
    ...
    while t <= clip_end:
        ...
        # Detectar rostos — sem margem de borda, tamanho mínimo pequeno
        faces = cascade.detectMultiScale(
            gray, scaleFactor=1.05, minNeighbors=4, minSize=(30, 30))

        # Filtrar apenas rostos no terço superior/médio (não no rodapé)
        valid = [(fx, fy, fw, fh) for (fx, fy, fw, fh) in faces
                 if fh >= 30 and fy < orig_h * 0.82]
        ...
        if len(valid) == 1:
            # Único rosto: usa direto
            fx, fy, fw, fh = valid[0]
            samples.append((offset, int(fx + fw // 2)))
        else:
            # Múltiplos rostos: escolhe quem tem mais movimento labial
            best_cx    = None
            best_score = -1
            ...
            for (fx, fy, fw, fh) in valid:
                ...
                # Região da boca: 55-100% vertical do rosto
                my1 = fy + int(fh * 0.55)
                my2 = fy + fh
                mx1, mx2 = fx, fx + fw
                ...
                # Movimento em relação ao frame anterior
                key = cx_face // 30  # agrupa por posição aproximada
                lip_score = 0
                ...
                # Score = movimento labial (peso 4) + proximidade ao centro (evita bordas)
                center_bonus = 100 - int(abs(cx_face - orig_w//2) / orig_w * 100)
                score = lip_score * 4 + fw * fh // 200 + center_bonus

                if score > best_score:
                    best_score = score
                    best_cx    = cx_face

            samples.append((offset, best_cx if best_cx else int(valid[0][0] + valid[0][2]//2)))

        t += interval

    cap.release()

    # ── FASE 2: Construir timeline de posições ──────────────────
    ...
    # Interpolação: preencher frames sem rosto com último cx válido
    last_good   = smoothed_cx[0]
    filled_cx   = {}
    si          = 0
    for t_off, cx_raw_v in samples:
        key = round(t_off, 1)
        if cx_raw_v is not None and si < len(smoothed_cx):
            last_good = smoothed_cx[si]
            si += 1
        filled_cx[key] = last_good

    # ── Construir segmentos de corte ─────────────────────────────
    # Calcular âncoras reais (não posições teóricas)
    ...
    def snap_to_anchor(cx):
        dl = abs(cx - anc_l)
        dr = abs(cx - anc_r)
        return anc_l if dl <= dr else anc_r

    MIN_DUR    = 5.0
    raw_segs   = []
    t_keys     = sorted(filled_cx.keys())
    seg_start  = 0.0
    cur_anchor = snap_to_anchor(filled_cx[t_keys[0]])

    for tk in t_keys[1:]:
        new_anchor = snap_to_anchor(filled_cx[tk])
        if new_anchor != cur_anchor:
            raw_segs.append({"start": seg_start, "end": tk, "cx": cur_anchor})
            seg_start  = tk
            cur_anchor = new_anchor

    raw_segs.append({"start": seg_start, "end": duration, "cx": cur_anchor})

    # Mesclar segmentos curtos ou com mesmo anchor
    segments = []
    for seg in raw_segs:
    ...
    for si, seg in enumerate(segments):
        ...
        x -= x % 2
        y  = 0  # vídeo 16:9 → crop ocupa toda altura
