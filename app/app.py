...
# ── Health check registrado IMEDIATAMENTE — responde antes de qualquer init ──
@app.route("/health")
def health():
    return {"status": "ok", "domain": "autoclip.up.railway.app"}
...
HAS_COOKIES = False  # será configurado no background

def recover_stale_jobs():
    """Marca jobs 'running' antigos como erro (Railway reiniciou)."""
    try:
        now = int(time.time())
        for p in JOBS_DIR.glob("*.json"):
            try:
                ...
                if j.get("status") == "running":
                    age = now - j.get("start_time", now)
                    if age > 30:
                        j["status"]  = "error"
                        j["message"] = "❌ Servidor reiniciou durante o processamento. Tente novamente."
...
def cleanup_old_files():
    ...
    - Vídeo fonte e áudio: apaga após 2 horas (não servem mais)
    - Clipes gerados: apaga após 48 horas (tempo para baixar)
    - Jobs .json: mantém para sempre (apenas 2KB cada)
    """
    try:
        import shutil
        now = time.time()
        ...
        # Apagar uploads antigos (vídeo fonte + áudio)
        for job_dir in UPLOADS.iterdir():
            if not job_dir.is_dir(): continue
            age = now - job_dir.stat().st_mtime
            if age > TWO_HOURS:
                shutil.rmtree(job_dir, ignore_errors=True)

        # Apagar clipes antigos mas marcar job como expirado
        for job_dir in OUTPUTS.iterdir():
            if not job_dir.is_dir(): continue
            age = now - job_dir.stat().st_mtime
            if age > FORTY_EIGHT_HOURS:
                shutil.rmtree(job_dir, ignore_errors=True)
                # Marcar job como expirado no .json
                job_json = JOBS_DIR / f"{job_dir.name}.json"
                if job_json.exists():
                    try:
                        j = json.loads(job_json.read_text())
                        if j.get("status") == "done":
                            j["expired"] = True
                            job_json.write_text(json.dumps(j))
                    except: pass

    except Exception as e:
        pass  # limpeza nunca deve quebrar o app


# Rodar tudo em background para não atrasar o startup
def _background_init():
    global HAS_COOKIES
    time.sleep(1)  # aguarda Flask subir e responder healthcheck
...
WHISPER_MODEL = os.environ.get("WHISPER_MODEL", "tiny")   # tiny usa ~300MB RAM (free tier)
USE_WHISPER  = os.environ.get("USE_WHISPER", "true").lower() == "true"

# Cache do modelo Whisper (carregado uma vez, reutilizado)
_whisper_model = None
...
def get_whisper_model():
    ...
    if _whisper_error:
        return None  # já falhou antes, não tentar de novo
    with _whisper_lock:
        if _whisper_model is None and not _whisper_error:
            try:
                ...
                _whisper_model = WhisperModel(
                    ...
                    device="cpu",
                    compute_type="int8",
                    cpu_threads=1,           # menos threads = menos RAM
                    num_workers=1,
...
def whisper_status():
    ...
    try:
        ...
        mem_avail = lines.get("MemAvailable", "?")
        mem_total = lines.get("MemTotal", "?")
    except: pass

    return jsonify({
        "faster_whisper_installed": fw_ok,
        "model_setting":    WHISPER_MODEL,
        "use_whisper":      USE_WHISPER,
        "model_loaded":     _whisper_model is not None,
        "load_error":       _whisper_error,
        "ram_available":    mem_avail,
        "ram_total":        mem_total,
        "model_cache_dir":  "/tmp/whisper_models",
    })
...
def run_pipeline(job_id, url, settings):
    # Registrar PID para detectar restart
    job_update(job_id, pid=os.getpid(), start_time=int(time.time()))
    # Forçar GC antes de iniciar
    ...
    try:
        ...
        # Tentar Whisper primeiro (gratuito), fallback para AssemblyAI
        if USE_WHISPER:
            transcript, raw_segments, words_data, tr_err = transcribe_whisper(audio_path, job_id)
            if not transcript and AAI_KEY:
                job_update(job_id, message="⚠️ Whisper falhou, tentando AssemblyAI...")
                transcript, raw_segments, words_data, tr_err = transcribe_assemblyai(audio_path, job_id)
        else:
            transcript, raw_segments, words_data, tr_err = transcribe_assemblyai(audio_path, job_id)
        
        if not transcript:
            return job_update(job_id, status="error", message=f"❌ Transcrição falhou: {tr_err}")
...
def transcribe_whisper(audio_path, job_id):
    """
    Transcrição local com faster-whisper (CTranslate2).
    ~300MB de RAM, sem GPU, sem custo de API.
    """
    try:
        from faster_whisper import WhisperModel
    except ImportError as ie:
        return None, [], [], f"faster-whisper não instalado: {ie}"
    ...
    try:
        ...
        try:
            with open("/proc/meminfo") as f:
                lines = {l.split(":")[0]: l.split(":")[1].strip() for l in f if ":" in l}
            mem_info = f" (RAM: {lines.get('MemAvailable','?')})"
        except: pass

        job_update(job_id,
            message=f"🎙️ Carregando Whisper {WHISPER_MODEL}...{mem_info}",
            whisper_model=WHISPER_MODEL)

        model = get_whisper_model()
        if not model:
            err = _whisper_error or "modelo não carregado"
            return None, [], [], f"Falha Whisper: {err}{mem_info}"

        job_update(job_id, message=f"🎙️ Transcrevendo com Whisper ({WHISPER_MODEL})...")

        # faster-whisper retorna generator de segmentos
        segments_gen, info = model.transcribe(
            str(audio_path),
            word_timestamps=True,
            vad_filter=True,
            language=None,
            beam_size=1,      # beam_size=1 = greedy, 3x mais rápido em CPU
        )

        job_update(job_id, whisper_lang=info.language,
                   whisper_lang_prob=round(info.language_probability, 2))
        ...
        for seg in segments_gen:
            ...
            # Atualizar progresso a cada 10s para manter job vivo
            if time.time() - last_update > 10:
                mins = int(seg.end // 60)
                job_update(job_id,
                    message=f"🎙️ Transcrevendo... {mins}min processados")
                last_update = time.time()

        full_text = " ".join(texts)
        if not full_text.strip():
            return None, [], [], "Whisper retornou texto vazio"

        job_update(job_id,
            whisper_segments=len(raw_segs),
            whisper_words=len(words_data),
            message=f"🎙️ {len(words_data)} palavras transcritas (Whisper local)")

        # Liberar modelo da RAM imediatamente após transcrição
        # Sem isso o FFmpeg não tem RAM suficiente para encodar
        global _whisper_model
        _whisper_model = None
        import gc; gc.collect()

        return full_text, raw_segs, words_data, None

    except Exception as e:
        return None, [], [], f"Whisper erro: {type(e).__name__}: {e}"
...
def generate_srt(words_data, clip_start, clip_end, srt_path, max_words=5, max_chars=35):
    """
    Gera SRT sincronizado com timestamps do Whisper.
    Regras para boa sincronia:
    - Máximo 5 palavras por bloco (menos palavras = mais fácil de ler)
    - Máximo 2.5s por bloco (blocos longos ficam dessincronizados)
    - Máximo 35 chars por linha
    - Compensação de drift do modelo tiny (-0.15s)
    """
    DRIFT_CORRECTION = 0.0   # ajuste se ainda dessincronizar (ex: -0.2)
    MAX_BLOCK_DUR   = 2.5    # segundos máximos por bloco

    clip_words = [w for w in words_data
                  if w.get("start") is not None and w.get("end") is not None
                  and (w["start"]/1000) >= clip_start - 0.5
                  and (w["start"]/1000) <= clip_end + 0.5]
    ...
    def flush():
        if buf and buf_s is not None:
            # Garantir duração mínima e corrigir drift
            s = max(0.0, buf_s + DRIFT_CORRECTION)
            e = max(s + 0.3, buf_e + DRIFT_CORRECTION)
            lines.append((s, e, " ".join(buf)))
    ...
    for w in clip_words:
        ...
        # Quebrar bloco se: muitas palavras, chars, ou duração longa
        block_dur = ws - buf_s if buf_s is not None else 0
        if (len(buf) >= max_words
                or len(" ".join(buf + [text])) > max_chars
                or block_dur > MAX_BLOCK_DUR):
            flush()
            buf = []
            buf_s = max(0.0, ws)
    ...
    for idx, (s, e, text) in enumerate(lines, 1):
        # Garantir que não há sobreposição com próximo bloco
        if idx < len(lines):
            next_s = lines[idx][0]
            e = min(e, next_s - 0.05)
...
def smart_silence_removal(input_path, output_path, job_id, clip_index):
    """
    Remove silêncios de forma natural — sem parecer repetição do locutor.

    Técnicas aplicadas:
    - Padding de 0.15s antes e 0.10s depois de cada segmento de fala
      (captura o ataque natural da voz e deixa a fala terminar)
    - Threshold conservador: -35dB e duração mínima 0.7s
      (só remove pausas longas, não hesitações naturais)
    - Re-encode com timestamps exatos via -af atrim
      (evita corte em meio-sílaba causado pelo -c copy)
    - Segmentos curtos são unidos ao vizinho
      (evita micro-segmentos que criam ritmo robótico)
    """
    try:
        # ── Parâmetros ajustáveis ────────────────────────────────
        NOISE_DB   = "-35dB"   # threshold: -35 = só silêncios claros
        MIN_SIL    = 0.7       # silêncio mínimo para remover (segundos)
        PAD_START  = 0.15      # contexto antes da fala (pega ataque da voz)
        PAD_END    = 0.10      # contexto depois da fala (deixa terminar)
        MIN_SEG    = 0.4       # segmento mínimo de fala (evita micro-cortes)
        MIN_SAVING = 0.08      # só processa se remover >8% do clipe

        # 1. Detectar silêncios com parâmetros conservadores
        probe = subprocess.run([
            "ffmpeg", "-i", input_path,
            "-af", f"silencedetect=noise={NOISE_DB}:d={MIN_SIL}",
            "-f", "null", "-"
        ...
        # 3. Montar segmentos de fala com padding natural
        raw_speech = []
        cursor = 0.0
        for ss, se in zip(silence_starts, silence_ends):
            if ss - cursor > MIN_SEG:
                raw_speech.append((cursor, ss))
            cursor = se
        if total_dur - cursor > MIN_SEG:
            raw_speech.append((cursor, total_dur))

        if len(raw_speech) <= 1:
            return input_path

        # Verificar economia suficiente
        total_speech = sum(e - s for s, e in raw_speech)
        if total_speech > total_dur * (1 - MIN_SAVING):
            return input_path

        # Aplicar padding — expandir cada segmento ligeiramente
        # (captura o início natural da voz e deixa ela terminar)
        padded = []
        for s, e in raw_speech:
            ps = max(0.0, s - PAD_START)
            pe = min(total_dur, e + PAD_END)
            padded.append((round(ps, 3), round(pe, 3)))

        # Mesclar segmentos sobrepostos após padding
        merged = [padded[0]]
        for s, e in padded[1:]:
            if s <= merged[-1][1] + 0.05:  # pequena margem para unir vizinhos
        ...
        # 4. Extrair segmentos com re-encode preciso
        # Usamos -af atrim para timestamps exatos (evita corte mid-sílaba)
        tmp_dir = Path(input_path).parent / f"sil_{clip_index}"
        tmp_dir.mkdir(exist_ok=True)
        seg_files = []

        # Detectar codec de vídeo do clipe
        probe_v = subprocess.run([
            "ffprobe", "-v", "quiet", "-print_format", "json",
            "-show_streams", "-select_streams", "v", input_path
        ], capture_output=True, text=True, timeout=15)
        try:
            vcodec = json.loads(probe_v.stdout)["streams"][0].get("codec_name", "h264")
        except:
            vcodec = "h264"

        for si, (s, e) in enumerate(merged):
            dur = e - s
            if dur < MIN_SEG:
                continue
            seg_out = tmp_dir / f"s{si:04d}.mp4"
            # Re-encode áudio com atrim para corte preciso no sample
            # Vídeo usa -c copy (rápido) — o áudio é o que causa o problema de repetição
            r = subprocess.run([
                "ffmpeg", "-y",
                "-ss", str(s), "-to", str(e),
                "-i", input_path,
                "-c:v", "copy",           # vídeo sem reencoder (rápido)
                "-af", f"atrim=0:{dur},asetpts=PTS-STARTPTS",  # áudio preciso
...
def analyze_with_claude(transcript, words_data, energy_data, settings, job_id):
    """
    Agente Claude multi-turno para análise profunda de conteúdo viral.

    TURNO 1 — Análise estratégica:
      Claude lê a transcrição e descreve o conteúdo, identifica os
      temas principais, picos emocionais, estrutura narrativa e
      potencial viral sem ainda escolher timestamps.

    TURNO 2 — Seleção precisa:
      Com base na própria análise, Claude escolhe os melhores clipes
      com timestamps precisos, justificativa e score.

    Resultado: muito mais preciso do que pedir tudo em um único prompt.
    """
    ...
    # Carregar memória do agente — tentar detectar perfil pelo canal
    active_profile_id = settings.get("profile_id", "default")
    agent_mem = agent_memory_read(active_profile_id)
    mem_inject = ""
    if agent_mem.get("context"):
        mem_inject += f"\n\nCONTEXTO DO CANAL: {agent_mem['context']}"
    if agent_mem.get("preferences"):
        mem_inject += f"\n\nPREFERÊNCIAS: {agent_mem['preferences']}"
    if agent_mem.get("instructions"):
        mem_inject += f"\n\nINSTRUÇÕES ESPECIAIS: {agent_mem['instructions']}"

    system_prompt = f"""Você é o ViralCut AI, um agente especialista em identificar momentos virais em vídeos de podcast, entrevistas e conteúdo educacional para redes sociais brasileiras.

Seu objetivo é encontrar os trechos que vão parar e fazer pessoas assistirem até o final, compartilharem e comentarem. Você entende de psicologia da atenção, storytelling e tendências de conteúdo no TikTok, Reels e YouTube Shorts.

Você analisa:
- Estrutura narrativa e arcos emocionais
- Momentos de tensão, surpresa, revelação ou virada
- Ganchos de abertura que prendem nos primeiros 3 segundos
- Pares pergunta-resposta com impacto
- Confissões, vulnerabilidade, polêmica controlada
- Picos de energia vocal (intensidade emocional){mem_inject}"""

    # ── TURNO 1 — Análise estratégica ───────────────────────────
    analysis_prompt = f"""Analise este conteúdo e me dê uma análise estratégica ANTES de escolher os clipes.

DADOS DO VÍDEO:
- Duração total: {int(words_data[-1]["end"]/1000) if words_data else 0}s
- Clipes desejados: {num} clipes de ~{target}s cada
{energy_summary}
...
Responda com:
1. TIPO DE CONTEÚDO: (podcast/entrevista/palestra/debate/etc)
2. TEMA PRINCIPAL: (resumo em 1 frase)
3. ESTRUTURA EMOCIONAL: (como a conversa evolui emocionalmente)
4. TOP 10 MOMENTOS CANDIDATOS: (timestamp + motivo, sem JSON ainda)
5. ESTRATÉGIA DE CORTE: (qual abordagem vai maximizar engajamento)"""

    try:
        job_update(job_id, message="🤖 Agente analisando estrutura do conteúdo...")
        r1 = requests.post(
        ...
        job_update(job_id, claude_status=r1.status_code)
        if r1.status_code != 200:
            return None, f"Claude T1 HTTP {r1.status_code}"

        analysis = r1.json()["content"][0]["text"].strip()
        job_update(job_id, claude_analysis=analysis[:500])

        # ── TURNO 2 — Seleção precisa com JSON ──────────────────
        job_update(job_id, message="🤖 Agente selecionando melhores momentos...")

        selection_prompt = f"""Perfeito. Agora, com base na sua análise, selecione exatamente {num} clipes finais.

REGRAS OBRIGATÓRIAS:
1. Início e fim SEMÂNTICO — nunca corte no meio de um raciocínio
2. Duração: mínimo {min_dur}s, máximo {max_dur}s, alvo {target}s
3. Os primeiros 3 segundos de cada clipe devem prender atenção imediatamente
4. Clipes não podem se sobrepor
...
{{
  "clips": [
    {{
      "start": 12.5,
      "end": 74.2,
      "hook": "frase de abertura que prende nos primeiros 3s",
      "score": 85,
      "reasoning": "por que este momento é viral",
      "energy_peak": true,
      "has_question_answer": false,
      "has_turning_point": true,
      "content_type": "revelação/debate/ensinamento/história/polêmica"
    }}
  ]
}}"""

        # Histórico de conversa completo para o turno 2
        r2 = requests.post(
            "https://api.anthropic.com/v1/messages",
            headers=HEADERS,
            json={
        ...
        if r2.status_code != 200:
            return None, f"Claude T2 HTTP {r2.status_code}"
...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    samples  = []   # (t_offset, cx)
    prev_mouths = {}
    interval = 1.0  # 1s entre amostras — menos RAM, mais rápido
    t = clip_start
    ...
    # Liberar MediaPipe da RAM antes do encode
    import gc; gc.collect()
    ...
    for si, seg in enumerate(segments):
        ...
        # force_original_aspect_ratio=disable garante resolução exata 1080x1920
        vf = f"crop={crop_w}:{crop_h}:{x}:0,scale={out_w}:{out_h}:flags=lanczos:force_original_aspect_ratio=disable,setsar=1"
    ...
    for i, clip in enumerate(clips):
        ...
        if has_srt and srt_path.exists():
            ...
            r4 = subprocess.run([
                ...
                "-threads","1","-x265-params","log-level=error",
                "-c:a","copy","-movflags","+faststart", str(out_path)
            ], capture_output=True, text=True, timeout=3600)
            full_log = (r4.stdout + r4.stderr)[-800:]
            job_update(job_id, **{f"sub_log_{i}": full_log})

            sub_ok = out_path.exists() and out_path.stat().st_size > 10000
            if not sub_ok:
                # Fallback: tentar com -vf subtitles sem force_style (mais compatível)
                job_update(job_id, **{f"sub_fallback_{i}": "tentando fallback simples"})
                srt_esc = str(srt_path).replace(":", "\\:")
                fs = max(10, min(32, int(caption_size)))
                mv = "60" if aspect == "9:16" else "40"
                simple_style = (f"subtitles={srt_esc}:force_style="
                    f"'FontSize={fs},Bold=1,PrimaryColour=&H00FFFFFF,"
                    f"OutlineColour=&H00000000,Outline=2,Shadow=1,"
                    f"MarginV={mv},Alignment=2'")
                r4b = subprocess.run([
                    "ffmpeg","-y","-i",working,"-vf",simple_style,
                    "-c:v","libx264","-preset","veryfast","-crf",crf,
                    "-profile:v","high","-pix_fmt","yuv420p",
                    "-threads","1",
                    "-c:a","copy","-movflags","+faststart", str(out_path)
                ], capture_output=True, text=True, timeout=1800)
                job_update(job_id, **{f"sub_log_{i}b": (r4b.stdout+r4b.stderr)[-400:]})
                sub_ok = out_path.exists() and out_path.stat().st_size > 10000

            if not sub_ok:
                # Último fallback: H264 (mínimo de RAM)
                r4c = subprocess.run([
                    "ffmpeg","-y","-i",working,
                    "-c:v","libx264","-preset","veryfast","-crf",crf,
                    "-profile:v","high","-pix_fmt","yuv420p",
            ...
            # Encode final H265 → fallback H264 se OOM
...
# ═══════════════════════════════════════════════════════════════
# AGENTE VIRALCUT AI — Interface de chat direta
# ═══════════════════════════════════════════════════════════════

# ── Sistema de perfis múltiplos ─────────────────────────────
PROFILES_FILE   = JOBS_DIR / "agent_profiles.json"
AGENT_CONV_FILE = JOBS_DIR / "agent_conv.json"

def profiles_read():
    """Lê todos os perfis salvos."""
    if PROFILES_FILE.exists():
        try: return json.loads(PROFILES_FILE.read_text())
        except: pass
    return {}

def profiles_save(profiles):
    PROFILES_FILE.write_text(json.dumps(profiles, ensure_ascii=False, indent=2))

def profile_get(profile_id="default"):
    ...
    return {
        ...
        "preferences":  "",
        "instructions": "",
        "channels":     [],  # canais vinculados ex: ["@NomeCanal"]
        "created_at":   int(time.time()),
    }

def profile_save(profile_id, data):
    """Salva um perfil."""
    profiles = profiles_read()
    data["id"] = profile_id
...
def profile_find_by_channel(channel_handle):
    """Busca perfil vinculado a um canal."""
    if not channel_handle:
        return None
    profiles = profiles_read()
    handle_clean = channel_handle.lower().lstrip("@")
    for pid, prof in profiles.items():
        for ch in prof.get("channels", []):
            if ch.lower().lstrip("@") == handle_clean:
                return prof
    return None

# Compatibilidade com código antigo
def agent_memory_read(profile_id="default"):
    return profile_get(profile_id)

def agent_memory_save(mem, profile_id="default"):
    profile_save(profile_id, mem)

def agent_conv_read():
    if AGENT_CONV_FILE.exists():
        try: return json.loads(AGENT_CONV_FILE.read_text())
        except: pass
    return []

def agent_conv_save(conv):
    AGENT_CONV_FILE.write_text(json.dumps(conv[-50:], ensure_ascii=False, indent=2))
...
def agent_chat():
    """Endpoint de chat com o agente ViralCut AI."""
    if not ANTHROPIC_KEY:
    ...
    memory = agent_memory_read(profile_id)
    conv   = agent_conv_read()

    # System prompt do agente com memória injetada
    memory_context = ""
    if memory.get("name") and memory.get("name") != "Padrão":
        memory_context += f"\n\nPERFIL ATIVO: {memory['name']}"
    if memory.get("context"):
        memory_context += f"\n\nCONTEXTO DO CANAL/NICHO:\n{memory['context']}"
    if memory.get("preferences"):
        memory_context += f"\n\nPREFERÊNCIAS DE CORTE:\n{memory['preferences']}"
    if memory.get("instructions"):
        memory_context += f"\n\nINSTRUÇÕES PERSONALIZADAS:\n{memory['instructions']}"
    if memory.get("channels"):
        memory_context += f"\n\nCANAIS VINCULADOS: {', '.join(memory['channels'])}"

    system = f"""Você é o ViralCut AI, agente especialista em criação de conteúdo viral para redes sociais brasileiras (TikTok, Reels, YouTube Shorts).

Você ajuda o usuário a:
- Definir estratégias para identificar momentos virais
- Entender o nicho e audiência do canal
- Configurar preferências de análise
- Analisar por que um clipe funciona ou não
- Sugerir melhorias no processo de criação de conteúdo
- Responder dúvidas sobre o AutoClipAI

Quando o usuário compartilhar informações sobre o canal, nicho ou preferências,
absorva essas informações e confirme que vai usar nas próximas análises.
{memory_context}"""
    ...
    try:
        ...
        if resp.status_code != 200:
            return jsonify({"error": f"Claude HTTP {resp.status_code}"}), 500

        reply = resp.json()["content"][0]["text"].strip()
        conv.append({"role": "assistant", "content": reply})
        agent_conv_save(conv)

        return jsonify({"reply": reply, "conv_length": len(conv)})
...
def agent_memory_get():
    ...
    return jsonify(agent_memory_read(profile_id))


@app.route("/agent/memory", methods=["POST"])
def agent_memory_update():
    """Salva contexto/preferências que o agente vai usar nas análises."""
    data       = request.json
    profile_id = data.get("profile_id", "default")
    memory     = agent_memory_read(profile_id)
    for key in ["name", "context", "preferences", "instructions", "channels"]:
        if key in data:
            memory[key] = data[key]
    agent_memory_save(memory, profile_id)
    return jsonify({"ok": True, "memory": memory})


@app.route("/agent/clear", methods=["POST"])
def agent_clear():
    """Limpa histórico de conversa (mantém memória)."""
    agent_conv_save([])
    return jsonify({"ok": True})



@app.route("/detect-channel", methods=["POST"])
def detect_channel():
    """Detecta canal do YouTube e retorna perfil vinculado."""
    url = (request.json or {}).get("url", "").strip()
    if not url:
        return jsonify({"channel": "", "channel_id": "", "profile": None})
    try:
        ...
        if result.stdout:
            try:
                ...
                channel_id = data.get("uploader_id", "") or data.get("channel_id", "")
                if channel_id and not channel_id.startswith("@"):
                    channel_id = "@" + channel_id
            except: pass
        profile = profile_find_by_channel(channel_id) or profile_find_by_channel(channel)
        return jsonify({"channel": channel, "channel_id": channel_id, "profile": profile})
    except Exception as e:
        return jsonify({"channel": "", "channel_id": "", "profile": None, "error": str(e)})


@app.route("/profiles", methods=["GET"])
def profiles_list():
    return jsonify(profiles_read())

@app.route("/profiles/<profile_id>", methods=["GET"])
def profile_get_route(profile_id):
    return jsonify(profile_get(profile_id))

@app.route("/profiles/<profile_id>", methods=["POST"])
def profile_save_route(profile_id):
    data = request.json or {}
    profile_save(profile_id, data)
    return jsonify({"ok": True, "profile": profile_get(profile_id)})

@app.route("/profiles/<profile_id>", methods=["DELETE"])
def profile_delete_route(profile_id):
    profiles = profiles_read()
    if profile_id in profiles and profile_id != "default":
