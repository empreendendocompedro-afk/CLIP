...
# ─── Job storage em arquivo (suporta múltiplos workers) ───────
def job_read(job_id):
    p = JOBS_DIR / f"{job_id}.json"
    if not p.exists(): return None
    with open(p) as f: return json.load(f)

def job_write(job_id, data):
    p = JOBS_DIR / f"{job_id}.json"
...
def run_pipeline(job_id, url, settings):
    try:
        ...
        if not audio_file.exists() or audio_file.stat().st_size < 1000:
            ...
            return job_update(job_id, status="error", message=f"❌ Áudio inválido ({size} bytes)")

        job_update(job_id, audio_size_kb=round(audio_file.stat().st_size/1024, 1),
                   progress=30, message="🎙️ Transcrevendo (AssemblyAI)...", stage="transcribe")

        transcript, raw_segments, aai_error = transcribe_assemblyai(audio_path, job_id)
        if not transcript:
            return job_update(job_id, status="error", message=f"❌ Transcrição falhou: {aai_error}")
        ...
        job_update(job_id, progress=86, message="⚡ Cortando e renderizando...", stage="render")
...
def download_video(url, job_id):
    ...
    for i, cmd in enumerate(strategies):
        ...
        job_update(job_id, **{f"dl_log_{i}": log})
...
def extract_audio(video_path, job_id):
    ...
    job_update(job_id, ffmpeg_log=log[-1000:])
...
def transcribe_assemblyai(audio_path, job_id):
    ...
    try:
        ...
        job_update(job_id, aai_upload_status=up.status_code,
                   aai_upload_resp=str(up.json())[:300])
        if up.status_code == 401:
            return None, [], "Chave inválida (401 Unauthorized)"
        if up.status_code != 200:
            return None, [], f"Upload falhou HTTP {up.status_code}: {up.text[:200]}"
        audio_url = up.json().get("upload_url")
        if not audio_url:
            return None, [], f"Sem upload_url: {up.json()}"
    except Exception as e:
        return None, [], f"Erro de conexão no upload: {e}"

    # Solicitar transcrição
    try:
        tr = requests.post(
            "https://api.assemblyai.com/v2/transcript",
            json={"audio_url": audio_url, "speech_models": ["universal-2"], "language_detection": True, "auto_chapters": True},
            headers={**headers, "content-type": "application/json"}, timeout=30
        )
        job_update(job_id, aai_transcript_status=tr.status_code,
                   aai_transcript_resp=str(tr.json())[:300])
        tid = tr.json().get("id")
        if not tid:
            return None, [], f"Sem transcript ID: {tr.json()}"
    except Exception as e:
    ...
    for attempt in range(120):
        try:
            ...
            job_update(job_id, aai_poll_status=status, aai_poll_attempt=attempt)
            if status == "completed":
                ...
                return None, [], f"AssemblyAI erro: {res.get('error','desconhecido')}"
        except Exception as e:
...
def debug(job_id):
    job = job_read(job_id)
    if not job: return jsonify({"error": "Job não encontrado — ID incorreto ou servidor reiniciado"}), 404
    return jsonify({k: v for k, v in job.items() if k != "clips"})

@app.route("/test-aai")
def test_aai():
    if not AAI_KEY:
        return jsonify({"ok": False, "erro": "ASSEMBLYAI_API_KEY não definida"})
    try:
        r = requests.get("https://api.assemblyai.com/v2/transcript",
                         headers={"authorization": AAI_KEY}, timeout=10)
        if r.status_code == 200:
            return jsonify({"ok": True, "msg": "Chave válida!"})
        elif r.status_code == 401:
            return jsonify({"ok": False, "erro": "Chave INVÁLIDA — 401 Unauthorized"})
        else:
