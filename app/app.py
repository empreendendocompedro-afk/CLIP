...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    JUMP = orig_w * 0.18  # 18% da largura = mudança de locutor
    segments = []
    seg_start = 0.0
    ...
    for i in range(1, len(smoothed)):
        ...
        if abs(cx - prev_cx) > JUMP:
            # Quebra detectada — fecha segmento atual
            avg_cx = int(sum(seg_cx_list) / len(seg_cx_list))
            segments.append({"start": seg_start, "end": t, "cx": avg_cx})
            seg_start = t
            seg_cx_list = [cx]
        else:
    ...
    if seg_cx_list:
        avg_cx = int(sum(seg_cx_list) / len(seg_cx_list))
        segments.append({"start": seg_start, "end": duration, "cx": avg_cx})
