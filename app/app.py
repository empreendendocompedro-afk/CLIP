...
def smooth_face_positions(face_timeline, window=3):
    """Suaviza posições de rosto com média móvel para eliminar jitter."""
    smoothed = []
    for i, entry in enumerate(face_timeline):
        if not entry["faces"]:
            smoothed.append(entry)
            continue
        # Janela de vizinhos com rostos
        neighbors = []
        for j in range(max(0, i-window), min(len(face_timeline), i+window+1)):
            if face_timeline[j]["faces"]:
                neighbors.append(face_timeline[j]["faces"][0])
        if neighbors:
            avg_cx = int(sum(f["cx"] for f in neighbors) / len(neighbors))
            avg_cy = int(sum(f["cy"] for f in neighbors) / len(neighbors))
            smoothed_faces = [{**entry["faces"][0], "cx": avg_cx, "cy": avg_cy}]
            smoothed.append({"time": entry["time"], "faces": smoothed_faces})
        else:
            smoothed.append(entry)
    return smoothed


def build_face_crop_segments(face_timeline, orig_w, orig_h, aspect, clip_duration):
    """
    Converte timeline de rostos em segmentos de crop.
    Aplica suavização antes de detectar saltos.
    Suporta alternância entre rostos para diálogos.
    """
    if not face_timeline:
        return []

    # Suavizar posições para evitar jitter
    face_timeline = smooth_face_positions(face_timeline)

    # Aspect ratio → dimensões do crop
    ratios = {"9:16": 9/16, "1:1": 1.0, "16:9": 16/9}
    ar     = ratios.get(aspect, 9/16)
    ...
    segments  = []
    prev_face = None  # (cx, cy, face_index)
    seg_start = 0.0
    JUMP_THRESHOLD = orig_w * 0.15  # 15% da largura = mudança de posição significativa
    ...
    for entry in face_timeline:
        ...
        if not faces:
            # Sem rosto detectado — mantém posição anterior
            continue

        # Selecionar rosto ativo:
        # Se havia diálogo (2+ rostos), alterna entre os dois maiores
        if len(faces) >= 2 and prev_face is not None:
            # Detectar se houve troca de locutor: escolhe o rosto mais diferente do anterior
            prev_cx, prev_cy = prev_face[0], prev_face[1]
            dists = [abs(f["cx"] - prev_cx) for f in faces[:2]]
            # Se o rosto secundário está mais longe, provavelmente é quem está falando
            active_idx = 1 if dists[1] > dists[0] and dists[1] > JUMP_THRESHOLD * 0.5 else 0
            active_face = faces[active_idx]
        else:
            active_face = faces[0]  # rosto principal (maior)

        cx = active_face["cx"]
        cy = active_face["cy"]

        # Clamp para não sair da imagem (sem ajuste aqui — feito em apply_face_tracking)
        cx = max(half_w, min(orig_w - half_w, cx))
        cy = max(half_h, min(orig_h - half_h, cy))

        # Verificar se houve salto significativo
        if prev_face is not None:
            dist = ((cx - prev_face[0])**2 + (cy - prev_face[1])**2) ** 0.5
            if dist > JUMP_THRESHOLD:
                # Fechar segmento anterior e abrir novo
                segments.append({
                    "start": seg_start,
                    "end":   t,
                    "cx":    prev_face[0],
...
def apply_face_tracking(video_path, out_path, clip_start, clip_end,
                        orig_w, orig_h, aspect, crf, job_id, clip_idx):
    """
    Aplica face tracking ao clipe:
    1. Detecta rostos por frame sampling
    2. Constrói segmentos de crop
    3. Gera sub-clipes para cada segmento de posição
    4. Concatena com cortes rápidos (sem crossfade)
    Retorna True se sucesso, False se fallback necessário.
    """
    out_path = Path(out_path)
    tmp_dir  = out_path.parent / f"face_{clip_idx}"
    tmp_dir.mkdir(exist_ok=True)

    duration = clip_end - clip_start

    # 1. Detectar rostos
    job_update(job_id, **{f"face_{clip_idx}_status": "detectando rostos..."})
    timeline = detect_faces_timeline(str(video_path), clip_start, clip_end, sample_interval=0.5)

    faces_found = sum(1 for t in timeline if t["faces"])
    face_pct    = round(faces_found / max(1, len(timeline)) * 100)
    # Logar primeira detecção para debug de coordenadas
    first_face = next((t["faces"][0] for t in timeline if t["faces"]), None)
    job_update(job_id, **{f"face_{clip_idx}_frames":   len(timeline),
                           f"face_{clip_idx}_detected": faces_found,
                           f"face_{clip_idx}_pct":      f"{face_pct}%",
                           f"face_{clip_idx}_first":    str(first_face)})

    # Se menos de 10% dos frames têm rosto → fallback para crop central
    if faces_found < len(timeline) * 0.10:
        job_update(job_id, **{f"face_{clip_idx}_status": f"poucos rostos ({face_pct}%) — crop central"})
        try: 
            import shutil
    ...
    # 2. Construir segmentos
    segments = build_face_crop_segments(timeline, orig_w, orig_h, aspect, duration)
    if not segments:
        return False

    # Aspect → dimensões alvo
    ratios  = {"9:16": 9/16, "1:1": 1.0, "16:9": 16/9}
    ar      = ratios.get(aspect, 9/16)
    crop_h  = orig_h
    crop_w  = int(orig_h * ar)
    ...
    if aspect == "9:16":
        ...
        out_w = out_h = min(720, crop_w * 2); out_w -= out_w % 2; out_h -= out_h % 2
    else:
    ...
    # 3. Gerar sub-clipe por segmento
    seg_files  = []
    ...
    for si, seg in enumerate(segments):
        ...
        # Posição de crop centrada no rosto
        cx  = int(seg["cx"])
        cy  = int(seg["cy"])

        # X: centraliza o crop horizontal no rosto
        x = cx - half_w
        x = max(0, min(orig_w - crop_w, x))
        x -= x % 2

        # Y: se há espaço vertical (vídeos > 9:16), sobe 20% para dar testa
        # Para vídeos 16:9 (crop_h == orig_h), y fica em 0
        head_offset = int(crop_h * 0.20) if (orig_h - crop_h) > 50 else 0
        y = cy - half_h - head_offset
        y = max(0, min(orig_h - crop_h, y))
        y -= y % 2
        y = max(0, y)
        ...
        # Log para debug
        job_update(job_id, **{f"face_{clip_idx}_seg{si}_crop":
            f"cx={cx} cy={cy} → crop={crop_w}x{crop_h}+{x}+{y} → {out_w}x{out_h}"})

        seg_path = tmp_dir / f"seg_{si:03d}.mp4"
    ...
    if len(seg_files) == 1:
        import shutil
        shutil.copy(str(seg_files[0]), str(out_path))
    else:
    ...
    job_update(job_id, **{f"face_{clip_idx}_status":
                           f"OK — {len(seg_files)} segmentos" if success else "FALHOU"})
    return success
...
def cut_clips(video_path, clips, job_id, settings, words_data):
    ...
    for i, clip in enumerate(clips):
        ...
        if use_face_track and not user_cropped:
            # Tentar face tracking — gera tmp_path diretamente
            job_update(job_id, message=f"🎯 Detectando rostos no clipe {i+1}...",
                       progress=87 + i)
            face_tracked = apply_face_tracking(
                video_path, tmp_path, cs, ce,
                orig_w, orig_h, aspect, crf, job_id, i
            )
