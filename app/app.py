...
def analyze_with_claude(transcript, words_data, energy_data, settings, job_id):
    ...
    # Carregar memória do agente para personalizar análise
    agent_mem = agent_memory_read()
    mem_inject = ""
...
AGENT_MEMORY_FILE = JOBS_DIR / "agent_memory.json"
AGENT_CONV_FILE   = JOBS_DIR / "agent_conv.json"

def agent_memory_read():
    """Lê a memória persistente do agente (contexto acumulado)."""
    if AGENT_MEMORY_FILE.exists():
        try: return json.loads(AGENT_MEMORY_FILE.read_text())
        except: pass
    return {
        "context":      "",   # contexto sobre o canal/nicho
        "preferences":  "",   # preferências de estilo de corte
        "topics":       [],   # tópicos/temas importantes
        "instructions": "",   # instruções personalizadas
    }

def agent_memory_save(mem):
    AGENT_MEMORY_FILE.write_text(json.dumps(mem, ensure_ascii=False, indent=2))

def agent_conv_read():
    """Lê histórico de conversa do agente."""
...
def agent_conv_save(conv):
    # Manter apenas últimas 50 mensagens para não estourar contexto
...
def agent_chat():
    ...
    if memory.get("topics"):
        memory_context += f"\n\nTÓPICOS IMPORTANTES: {', '.join(memory['topics'])}"
...
def agent_memory_get():
    return jsonify(agent_memory_read())
...
def agent_memory_update():
    ...
    data   = request.json
    memory = agent_memory_read()
    for key in ["context", "preferences", "instructions"]:
        if key in data:
            memory[key] = data[key]
    if "topics" in data:
        memory["topics"] = data["topics"]
    agent_memory_save(memory)

    # Reinjetar na próxima análise automaticamente
