...
def smart_silence_removal(input_path, output_path, job_id, clip_index):
    """
    Remove silêncios com precisão de sample — sem repetição de frases.

    Usa FFmpeg select/aselect filters em vez de concat demuxer.
    Resultado: corte exato no segundo que começa o silêncio e
    retomada exata no segundo que começa a fala.
    """
    try:
        # ── Parâmetros ───────────────────────────────────────────
        NOISE_DB  = -38        # threshold em dB (mais negativo = mais sensível)
        MIN_SIL   = 0.6        # silêncio mínimo para remover (segundos)
        MIN_SAVE  = 0.08       # só processa se remover >8% do clipe
        ...
        # 3. Montar intervalos de FALA (inverso dos silêncios)
        speech = []
        cursor = 0.0
        for ss, se in zip(silence_starts, silence_ends):
            seg_dur = ss - cursor
            if seg_dur > 0.1:
                speech.append((round(cursor, 4), round(ss, 4)))
            cursor = se
        rem = total_dur - cursor
        if rem > 0.1:
            speech.append((round(cursor, 4), round(total_dur, 4)))

        if not speech or len(speech) <= 1:
            return input_path

        # Verificar economia mínima
        total_speech = sum(e - s for s, e in speech)
        if total_speech > total_dur * (1 - MIN_SAVE):
            return input_path

        # 4. Construir expressão select para vídeo e aselect para áudio
        # Cada condição: between(t, START, END)
        # Isso garante corte EXATO no timestamp, não no keyframe
        conditions = "+".join(
            f"between(t,{s},{e})" for s, e in speech
        )

        # 5. Encode com filtros select/aselect — uma única passagem FFmpeg
        # sem concat, sem copy — corte preciso em ambas as streams
        r = subprocess.run([
        ...
        if Path(output_path).exists() and Path(output_path).stat().st_size > 5000:
            job_update(job_id, **{
                f"sil_{clip_index}_segs": len(speech),
                f"sil_{clip_index}_saved": f"{(1 - total_speech/total_dur)*100:.1f}%"
            })
