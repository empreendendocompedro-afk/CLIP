...
def remux_video(video_path, job_id):
    ...
    if out_path.exists() and out_path.stat().st_size > 100000:
        # Remove o original e retorna o limpo
        try: Path(video_path).unlink()
        except: pass
        return str(out_path)
    else:
        # Falhou — continua com o original
...
def cut_clips(video_path, clips, job_id, settings, words_data):
    ...
    for i, clip in enumerate(clips):
        ...
        cmd1 = (["ffmpeg","-y","-ss",str(cs),"-i",video_path,"-t",str(duration),
        ...
        job_update(job_id, **{f"cut_log_{i}": (r1.stdout+r1.stderr)[-400:]})
