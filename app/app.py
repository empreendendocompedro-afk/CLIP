...
def detect_and_track_faces(video_path, clip_start, clip_end, orig_w, orig_h,
    ...
    while t <= clip_end:
        ...
        if len(faces) > 0:
            # Maior rosto (mais próximo da câmera)
    ...
    # Separar posições em dois grupos: esquerda e direita do frame
    # Threshold: metade da largura do vídeo
    mid = orig_w // 2
    left_positions  = [cx for _, cx in smoothed if cx < mid]
    right_positions = [cx for _, cx in smoothed if cx >= mid]
    ...
    # Se só um locutor detectado, usar posição única
    ...
    def snap(cx):
        """Retorna o anchor (left ou right) mais próximo do cx."""
        dl = abs(cx - anchor_left)  if anchor_left  is not None else 99999
        dr = abs(cx - anchor_right) if anchor_right is not None else 99999
        return anchor_left if dl <= dr else anchor_right

    raw_segs    = []
    seg_start   = 0.0
    cur_anchor  = snap(smoothed[0][1])
