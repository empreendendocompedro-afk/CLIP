...
def extract_audio(video_path, job_id):
    ...
    r = subprocess.run([
        ...
        "-ar","16000","-ac","1","-vn","-b:a","32k","-t","1800",str(audio_path)
...
def transcribe_assemblyai(audio_path, job_id):
    ...
    for attempt in range(240):  # 20 minutos
        try:
            res = requests.get(f"https://api.assemblyai.com/v2/transcript/{tid}",
                               headers=headers, timeout=30).json()
            st = res.get("status")
            # Atualizar mensagem de progresso a cada 10 tentativas
        ...
        time.sleep(5)
    return None, [], "Timeout (20 min) — vídeo muito longo. Tente um vídeo menor que 30 minutos."
