...
def detect_faces_timeline(video_path, clip_start, clip_end, sample_interval=0.5):
    ...
    while t <= clip_end:
        ...
        if len(faces) > 0:
            ...
            # Ordenar por tamanho (maior = mais próximo da câmera)
...
def build_face_crop_segments(face_timeline, orig_w, orig_h, aspect, clip_duration):
    ...
    for entry in face_timeline:
        ...
        # Subir enquadramento: olhos devem ficar no terço superior (regra dos terços)
        # Adiciona offset de -25% da altura do crop para dar espaço à testa
        head_margin = int(crop_h * 0.22)
        cy = cy - head_margin   # sobe o enquadramento
...
def apply_face_tracking(video_path, out_path, clip_start, clip_end,
    ...
    for si, seg in enumerate(segments):
        ...
        # Posição de crop: rosto no terço superior (olhos ~30% do topo)
        cx  = int(seg["cx"])
        cy  = int(seg["cy"])
        head_offset = int(crop_h * 0.22)   # sobe 22% da altura para dar espaço à testa
        x   = max(0, min(orig_w - crop_w, cx - half_w)); x -= x % 2
        y   = max(0, min(orig_h - crop_h, cy - half_h - head_offset)); y -= y % 2
        y   = max(0, y)  # não pode ser negativo
